...
public interface PhotoReviewRepository extends JpaRepository<PhotoReview, Long> {
    ...
    @Query(nativeQuery = true, value = "SELECT R.ID AS REVIEWID, PS.PHOTO_REVIEW_ID AS SUBMISSIONREVIEWID, P.ID AS SUBMISSIONID, R.* FROM PHOTO_REVIEWS R JOIN PHOTO_SUBMISSION_REVIEWS PS ON R.ID = PS.PHOTO_REVIEW_ID JOIN PHOTO_SUBMISSIONS P ON PS.PHOTO_SUBMISSION_ID = P.ID WHERE R.JURY_ID = :userId and P.ID=:photoSubmissionId")
